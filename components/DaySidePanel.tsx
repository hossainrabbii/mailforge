...
export function DaySidePanel({ date, emails, onClose }: DaySidePanelProps) {
  return (
    <div className="w-auto shrink-0 animate-in slide-in-from-right-4 duration-200">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4">
              {emails.map((email, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 rounded-lg border bg-background p-3 transition-colors hover:bg-muted/50 max-w-[300px]"
                >
                    ...
                    <p className="text-xs text-gray-400 mb-2">
                      {email.currentUrl ? (
                        <Link href={email.currentUrl as string}>
                          {email.currentUrl.length > 10
                            ? `${email.currentUrl.slice(0, 30)}...`
                            : email.currentUrl}
                        </Link>
                      ) : (
                        <>—</>
